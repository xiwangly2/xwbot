...
class ChatAIProcess:
    ...
    def run(self):
        """独立进程的主逻辑"""
        while True:
            print(f"热度值: {self.heat_value}")
            if not self.input_queue.empty():
                messages, message = self.input_queue.get()
                self._update_heat_value(messages, message)
                if self.heat_value > 0 or f"[CQ:at,qq={messages['self_id']}]" in message:
                    response = self._generate_response(messages, message)
    ...
    def _update_heat_value(self, messages, message):
        ...
        if f"[CQ:at,qq={messages['self_id']}]" in message:
            self.heat_value = min(500, self.heat_value + 50)  # 热度值上限为 500

    def _generate_response(self, messages, message):
        """生成 AI 回复"""
